...
class Runnable:
    ...
    hooks: dict = dataclasses.field(default_factory=lambda: {"enter": [], "exit": []})
    ...
    stamp: dict = dataclasses.field(
        default_factory=lambda: {"begin": None, "end": None}
    )
    ...
    def enter(self, trun):
        ...
        if trun.args.verbose:
            cij.emph("rnr:enter { ident: %r, rcode: %r } " % (self.ident, rcode))
    ...
    def exit(self, trun):
        ...
        if trun.args.verbose:
            cij.emph("rnr:exit: { ident: %r, rcode: %r }" % (self.ident, rcode))
...
class TestSuite(Runnable):
    ...
    progress: dict = dataclasses.field(
        default_factory=lambda: {Status.Pass: 0, Status.Fail: 0, Status.Unkn: 0}
    )
    status_preq: str = Status.Unkn
...
class TestRun(Runnable):
    ...
    conf: cij.conf.Config = dataclasses.field(default_factory=cij.conf.Config)
    counter: int = 0
    progress: dict = dataclasses.field(
        default_factory=lambda: {Status.Pass: 0, Status.Fail: 0, Status.Unkn: 0}
    )
    ...
    def from_dict(trun_dict) -> TestRun:
        ...
        complex_keys = {"testplans": TestPlan.tplans_from_dicts, "args": args_from_dict}
...
def script_run(trun: TestRun, script: Runnable):
    ...
    with open(script.log_fpath, "a", encoding="UTF-8") as log_fd:
        ...
        with Popen(
            cmd, stdout=log_fd, stderr=STDOUT, cwd=script.res_root, env=evars
        ) as process:
            process.wait()
...
def trun_to_file(trun: TestRun, fpath=None):
    ...
    def dict_factory(instance):
        ...
        return dict((x[0], vars(x[1])) if x[0] == "args" else x for x in instance)
...
def trun_to_junitfile(trun: TestRun, fpath=None) -> int:
    ...
    try:
        ...
        duration = max(
            0, (trun.stamp["end"] or time.time()) - (trun.stamp["begin"] or time.time())
        )
        ...
        for tplan in trun.testplans:
            for tsuite in tplan.testsuites:
                ...
                doc_tsuite.setAttribute("tests", str(len(tsuite.testcases)))
                ...
                for tcase in tsuite.testcases:
                    ...
                    doc_tcase.setAttribute("name", str(tcase.name))
                    doc_tcase.setAttribute("classname", str(tcase.ident))
...
def tcase_setup(trun: TestRun, parent, tcase_fname) -> TestCase:
    ...
    if not os.path.exists(case.fpath_orig):
        msg = (
            "rnr:tcase_setup: file case.fpath_orig does not exist: "
            "%r" % case.fpath_orig
        )
        cij.err(msg)
        raise InitializationError(msg)
...
def tsuite_setup(trun: TestRun, tplan: TestPlan, declr, enum) -> TestSuite:
    ...
    if os.path.exists(suite.fpath):  # From suite-file
        with open(suite.fpath, encoding="UTF-8") as sfd:
            suite_lines = (line.strip() for line in sfd.read().splitlines())
            tcase_fpaths.extend(
...
def trun_setup(args: argparse.Namespace, conf: cij.conf.Config) -> TestRun:
    ...
    trun.testplans = [tplan_setup(trun, tp_fpath) for tp_fpath in args.testplans]
    ...
    for tplan in (tp for tp in trun.testplans if trun.entered):
        ...
        for tsuite in (ts for ts in tplan.testsuites if tplan.entered):
            ...
            for tcase in (tc for tc in tsuite.testcases if tsuite.entered):
                if args.testcase_match is None or args.testcase_match in tcase.name:
                    tcase.enter(trun)
                    if tcase.entered:
