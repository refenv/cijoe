...
- An individual :ref:`script <sec-resources-scripts>` to execute **or** a
  ...
  in a  :ref:`task <sec-resources-tasks>`
...
example script, task, and configuration file generated by running:
...
:ref:`sec-resources-tasks`, the remainder of the current section
...
(``--c / --config``) by default search for files named ``cijoe-task.yaml``
...
CIJOE_DEFAULT_WORKFLOW
    Deprecated alias for ``CIJOE_DEFAULT_TASK``. Honored when
    ``CIJOE_DEFAULT_TASK`` is unset; emits a deprecation warning.
