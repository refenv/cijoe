...
# Required variables
# 
# TFTP_KERNEL_PATH -  scp path to where thee tftp server picks up the kernel image
#		     i.e tfp-server:/srv/tftpboot/pxelinux.bzi/testmachine.bzImage
...
function tftp::provision_kernel
{
  tftp::env
  if [[ $? -ne 0 ]]; then
