...
function pblk::trace_enable
{
  ssh::cmd "echo 1 > /sys/kernel/debug/tracing/events/pblk/pblk_$1/enable"
}

function pblk::trace_disable
{
  ssh::cmd "echo 0 > /sys/kernel/debug/tracing/events/pblk/pblk_$1/enable"
}

function pblk::trace_watch
{ 
  ssh::cmd_t "watch -n.5 \" \
	  tail -n \$(( \$(tput lines) - 2 )) /sys/kernel/debug/tracing/trace\""
}

function pblk::trace_all
{
